...
class EventList:
    ...
    Instance Attributes:
        ...
        - # first: The first event in this EventList, or None if the list is empty
        - # last: The last event in this EventList, or None if the list is empty

    Representation Invariants:
        - # TODO add any appropriate representation invariants, if needed
        - # if first == None, then last == None
    """
    ...
    #  TODO: Complete the methods below, based on the given descriptions. Do NOT change any of their specifications.
    ...
    def add_event(self, event: Event, command: str = None) -> None:
        ...
        # if there are already events in this event list
        if self.last:
            self.last.next_command = command
            self.last.next = event
            temp_event = self.last
            self.last = event
            self.last.prev = temp_event
        # if we are adding out first event
        else:
            self.first = event
            self.last = event
